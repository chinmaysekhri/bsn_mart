...
<div>
   <ul class="flex space-x-2 rtl:space-x-reverse">
      <li>
         <a href="{{route('buyer_profile')}}" class="text-primary hover:underline">Edit Buyer</a>
      </li>
                  ...
                  <div class="flex flex-col sm:flex-row">
					...
					<img src="{{$profile_img}}" alt="profile-default-image" class="mx-auto h-20 w-20 rounded-full object-cover md:h-32 md:w-32" />
                     ...
                     <div class="grid flex-1 grid-cols-1 gap-5 sm:grid-cols-2">
                      <div>
                        ...
                        <input id="date_of_enrollment" type="text" name="date_of_enrollment"  value="{{$buyer->date_of_enrollment}}" placeholder="Customer ID" class="form-input"/>
                     </div>
                   ...
                   <!--<div>
                     ...
                     <div>
                        ...
                        <input id="business_name" type="text" name="business_name"  value="{{$buyer->business_name}}" placeholder=" Enter Business Name" class="form-input"/>
                     </div>
                    
                     <div>
                        <label for="name">First Name</label>
                        <input id="first_name" type="text" name="first_name" value="{{$buyer->first_name}}"  placeholder="Enter First Name" class="form-input"/>
                     </div>
                     <div>
                        <label for="name">Last Name</label>
                        <input id="last_name" type="text" name="last_name" value="{{$buyer->last_name}}" placeholder="Enter Last Name" class="form-input"/>
                     </div>
                     <div>
                        <label for="name">Contact</label>
                        <input id="mobile" name="mobile" value="{{$buyer->mobile}}"  type="text" placeholder="Enter Mobile No" class="form-input"/>
                     </div>
                     <div>
                        <label for="name">Email</label>
                        <input id="Email" type="text" name="email" value="{{$buyer->email}}" placeholder="Enter Email Address" class="form-input"/>
                     </div>
					 ...
					 <div class="flex justify-around pt-5">
                      ...
                      <div>
                        ...
                        <input id="present_address" type="text" name="present_address" value="{{$buyer->present_address}}" placeholder="Enter Present Address" class="form-input">
                     </div>
					 
						<div>
						 <label for="pin_code">Pin Code</label>
							
						 <input id="pin_code" type="number" name="pin_code" value="{{$buyer->pin_code}}" placeholder="Enter Pin Code" class="form-input" onkeyup="getStateCity(this);"/>
						</div>
						
						<div>
						   <label for="country">Country</label>
						   <input id="country" type="text" name="country" value="{{$buyer->country}}" placeholder="Enter Email Address" class="form-input"/>
						</div>
						<div>
                     ...
                     <form action="{{route('update_buyer_profile', Auth::user()->buyer_id)}}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
                           ...
                           <div>
                              ...
                              <input id="aadhar_no" type="text" name="aadhar_no" value="{{$buyer->aadhar_no}}" class="form-input" /><br><br>
                              <input id="upload_aadhar_no" type="file" name="upload_aadhar_no" value="{{$buyer->upload_aadhar_no}}" class="form-input" />
                           @if(!empty($buyer->upload_aadhar_no))
                           <a href="{{asset('public/uploads/buyer/upload_aadhar_no/'.$buyer->upload_aadhar_no)}}" target="_blank">
                           <img class="h-9 w-9 rounded-full object-cover saturate-50 group-hover:saturate-100" src="{{asset('public/uploads/buyer/upload_aadhar_no/'.$buyer->upload_aadhar_no)}}" ></a>
                             @endif
                           </div>
                           <div>
                              <label for="pan_no">Pan No</label>
                              <input id="pan_no" type="text" name="pan_no" value="{{$buyer->pan_no}}" class="form-input" /><br><br>
                              <input id="upload_pan_no" type="file" name="upload_pan_no" value="{{$buyer->upload_pan_no}}" class="form-input" />
                               @if(!empty($buyer->upload_pan_no))
                           <a href="{{asset('public/uploads/buyer/upload_pan_no/'.$buyer->upload_pan_no)}}" target="_blank">
                           <img class="h-9 w-9 rounded-full object-cover saturate-50 group-hover:saturate-100" src="{{asset('public/uploads/buyer/upload_pan_no/'.$buyer->upload_pan_no)}}" ></a>
                             @endif
                           </div>
                           <div>
                              <label for="gst_no">GST No</label>
                              <input id="gst_no" type="text" name="gst_no" value="{{$buyer->gst_no}}" class="form-input" /><br><br>
                              <input id="upload_gst_no" type="file" name="upload_gst_no" value="{{$buyer->upload_gst_no}}" class="form-input" />
                               @if(!empty($buyer->upload_gst_no))
                           <a href="{{asset('public/uploads/buyer/upload_gst_no/'.$buyer->upload_gst_no)}}" target="_blank">
                           <img class="h-9 w-9 rounded-full object-cover saturate-50 group-hover:saturate-100" src="{{asset('public/uploads/buyer/upload_gst_no/'.$buyer->upload_gst_no)}}" ></a>
                             @endif
                           </div>
                     ...
                     <form action="{{route('update_buyer_profile', Auth::user()->buyer_id)}}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
                           ...
                           <div>
                              ...
                              <input id="bank_name" type="text" name="bank_name" value="{{$buyer->bank_name}}" placeholder="Enter Bank Name"
                           class="form-input" />
                           </div>
                           <div>
                              <label for="ifsc_code">IFSC Code</label>
                              <input id="ifsc_code" type="text" name="ifsc_code" value="{{$buyer->ifsc_code}}" placeholder="Enter IFSC Code"
                           class="form-input" />
                           </div>
                           <div>
                              <label for="account_no">Bank Account No</label>
                               <input id="account_no" type="text" name="account_no" value="{{$buyer->account_no}}" placeholder="Enter Bank Account No"
                           class="form-input" />
                           </div>
               ...
               <div class="panel">
                  ...
                  <div class="mb-5">
                     <form action="{{route('update_buyer_profile', Auth::user()->buyer_id)}}" method="post" enctype="multipart/form-data">
					   ...
					   <button type="submit" class="btn btn-info !mt-6" style="display: inline-flex;   float: inline-end;"><a href="{{route('buyer_profile')}}" >Back To Profile</a></button>
                     </form>
                  </div>
               </div>
            </div>
