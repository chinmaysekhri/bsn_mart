...
@section('title','Edit Prospective Buyer')
@section('content')
...
<div class="grid grid-cols-1 gap-6 pt-5 lg:grid-cols-1">
   <div class="panel">
      <div class="mb-5">
         ...
         @endif
           {!! Form::model($pros_buyer, ['method' => 'PATCH','route' => ['prospectivebuyers.update', $pros_buyer->id],'class'=>'space-y-5', 'enctype'=>'multipart/form-data']) !!}
			...
			<!--31-05-2024 for hold value after serch next action date End-->
      ...
      @endphp
           ...
           <!-- <div>
             ...
             <div>
               ...
               {!! Form::select('state',[$pros_buyer->state=>$pros_buyer->state],[], array('class' => 'form-select text-white-dark','id'=>'state')) !!}
            </div>
            <div>
               <label for="city">City</label>
                {!! Form::select('city', [$pros_buyer->city=>$pros_buyer->city],[], array('class' => 'form-select text-white-dark','id'=>'city')) !!}
            </div>
             <div>
               <label for="city">Status</label>
			   <select class="form-select" id="Status"  name="status_name" onchange="change_status();">
                 <option value="" >Select Status</option>
                  <option value="Pending" @if($pros_buyer->status_name == "Pending") selected @endif >Pending</option>
                    <option value="Interested" @if($pros_buyer->status_name == "Interested") selected @endif>Interested</option>
                    <option value="Not Interested" @if($pros_buyer->status_name == "Not Interested") selected @endif>Not Interested</option>
                    <option value="Onboarded" @if($pros_buyer->status_name == "Onboarded") selected @endif>Onboarded</option>
                  </select>
            </div>
			...
			<div class="" id="payment_info_hide" style="display: @if($pros_buyer->status_name == "Pending"  || $pros_buyer->status_name == "Interested" || $pros_buyer->status_name == "Onboarded")block @else none  @endif;">
                  <label for="date_of_enrollment"><i class="fa fa-asterisk" style="font-size:6px;color:red"></i>Next Action Date
                   </label>
                   <input type="date" id="" name="date_of_enrollment" class="form-input" value="{{$pros_buyer->date_of_enrollment}}"> 
