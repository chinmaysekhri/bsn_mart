...
@section('title','Prospective Buyer List')
@section('content')
<div x-data="form">
   <div class="panel">
      <!-- Flash  Message  start -->
      ...
      <h5 class="mb-5 text-lg font-semibold dark:text-white-light md:absolute md:top-[25px] md:mb-0"> 
       @can('prospective-buyer-create')             
         <a class="badge bg-success" href="{{route('prospectivebuyers.create')}}">Create Prospective Buyer</a>
     
         @endcan
		 
		 
      <a href="{{route('prospectivebuyers.index')}}"  class="btn btn-primary" style="margin-left: 775px;margin-top: -24px; padding: 1px 16px 1px 16px;">Reset Search</a>

       <a href="#"><button  class="btn btn-info" onclick="mystatusFunction()" style="margin-left:450px;margin-top: -24px;padding: 1px 16px 1px 16px;">Search By</button></a>
     </h5>
	
	
      <div class="dataTable-wrapper dataTable-loading no-footer sortable searchable fixed-columns">
          <form method="GET" action="{{route('prospectivebuyers.index')}}">

            <div class="mb-5" id="mystatusDIV" style="display: none;margin-top: 30px;">
                 
               <div class="mb-5 grid grid-cols-1 gap-4 sm:grid-cols-5">
                  <label style="margin-top:50px;">Applied On:</label>
                  <div style="margin-top:35px;">
                  <a href="{{route('prospectivebuyers.index',['today_applied_on'=>date('Y-m-d')])}}" class="btn btn-outline-secondary" style="padding-right:40px;padding-left:40px;" >Today</a></div>
                ...
                <div class="mb-5 grid grid-cols-1 gap-4 sm:grid-cols-5">
                    ...
                    <div style="margin-top:35px;">
                  <a href="{{route('prospectivebuyers.index',['today_updated_on'=>date('Y-m-d')])}}" class="btn btn-outline-secondary" style="padding-right:40px;padding-left:40px;">Today</a></div>
                  <div class="" style="margin-top: 7px;">
                  ...
                  <div style="margin-top:35px;">
                  <a href="{{route('prospectivebuyers.index',['today_next_action'=>date('Y-m-d')])}}" class="btn btn-outline-secondary" style="padding-right:40px;padding-left:40px;" >Today</a></div>
                  <div class="" style="margin-top: 7px;">
			    ...
			    <!--08-02-2025 -->
				  <div style="margin-top: 36px;">
                     <div class="search-date-group ms-5 d-flex align-items-center">
                        <select class="form-input" name="team_member_data" id="team_member_data" value="">
                ...
                <!--08-02-2025 -->
                 ...
                 <div style="margin-top: 31px;">
                     <button type="submit" class="btn btn-outline-success" style="padding-right: 64px;padding-left:64px; margin-left: 445px;" >Submit</button>
                  </div>
                   </div>
                   <hr>
            </div>
		...
		<!-- Assign Leads for customer 08-02-2025 -->
         ...
         <div class="mb-5 flex items-center justify-between" style="margin-left:173px;margin-bottom: -18px;
            margin-top: 8px;">
            <button type="button" class="btn btn-primary" @click="toggle" style="padding: 2px 19px 2px 19px; margin-top: -19px;">Assign To</button>
         </div>
               ...
               <div class="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8 animate__animated" :class="$store.app.rtlClass === 'rtl' ? 'animate__fadeInLeft' : 'animate__fadeInRight'">
                  ...
                  <div class="p-5">
                     <div class="dark:text-white-dark/70 text-base font-medium text-[#1f2937]">
					     ...
					     <form class="space-y-5" action="{{route('prospective_buyers_assign_employee')}}" method="post" enctype="multipart/form-data">
                           @csrf
                           ...
                           <input type="hidden" name="prospective_buyer_data" value="" id="prospective_buyer_data">
      ...
      <!-- Assign Leads for customer 08-02-2025 -->
         ...
         <div class="dataTable-container">
            <table id="myTable1" class="whitespace-nowrap dataTable-table" style="margin-top:60px;">
               ...
               <tbody>
                  ...
                  $i=1;
                  @endphp
                 @foreach ($prosBuyerData as $key => $pros_buyer)
                  <tr>
				     <td scope="row">
                        <div class="form-check">
                           <input  type="checkbox"  name="appclient" class="appclient-input" value="{{ $pros_buyer->id }}" onclick="getAppclient();">
                        </div>
                     </td>
                     <td>{{ $i++ }}</td>
                     <td>JBNB000{{ $pros_buyer->id }}</td> 
                     <td>{{ $pros_buyer->date_of_enrollment }}</td>
                      <td>{{ $pros_buyer->business_name }}</td>
                     <td>{{ $pros_buyer->first_name.' '.$pros_buyer->last_name}}</td>
                     <td>{{ $pros_buyer->email }}</td>
						@php
						
						 if(!empty($pros_buyer->assign_to)){
			 
						   $getAssignUserName = Helper::getAssignUserNameData($pros_buyer->assign_to);
                      ...
                      <td>{{ $pros_buyer->contact }}</td>
                      <td>{{ $pros_buyer->status_name }}</td>
                      <td>
                        <a class="badge bg-info" href="{{ route('prospectivebuyers.show',$pros_buyer->id) }}">Show</a>
                        
						@can('prospective-buyer-edit')
						 
						 <!--31-01-2025 for hold value after search next action date -->
						 
						 <a  class="badge bg-primary" href="{{ route('prospectivebuyers.edit',array_merge([$pros_buyer->id],$requested_input)) }}">Edit</a>
                       
						@endcan
						 
                        @can('prospective-buyer-delete')
                      
                        {!! Form::open(['method' => 'DELETE','route' => ['prospectivebuyers.destroy', $pros_buyer->id],'style'=>'display:inline', 'id'=>"form".$pros_buyer->id]) !!}
                        {!! Form::button('Delete', ['class' => 'badge bg-danger', 'onclick'=>"confirmDelete($pros_buyer->id)"]) !!}
                        {!! Form::close() !!}
                         @endcan
                     </td>
                  </tr>
       ...
       {{ $prosBuyerData->links('admin.partials.pagination')}}
      </div>
   </div>
</div>
@endsection
...
function checkBoxHandle (){
   ...
   $('#prospective_buyer_data').val(checkedVals.join(","));
