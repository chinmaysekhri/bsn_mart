...
<div x-data="form">
    <ul class="flex space-x-2 rtl:space-x-reverse">
        <li>
            <a href="{{route('my_product_order')}}" class="text-primary hover:underline">Return My Product Order</a>
        </li>
   ...
   <div class="panel">
      <div class="dataTable-wrapper dataTable-loading no-footer sortable searchable fixed-columns">
        ...
        <form action="{{route('cartreturns.store')}}" method="post" id="checkout_form">
		        ...
		        <input type="hidden" id="product_total_{{$id}}_qty" name="product_total_{{$id}}_qty" value="{{ $details['product_total_qty'] }}">
                  ...
                  <tr data-id="{{ $id }}">
				    ...
				    <td style="display:none;">
                     ...
                     <td>
                      <input type="hidden" class="single_product_subtotal" id="prod_id_{{$id}}_subtotal_price_val" value="{{ ($details['price']) }}">
                      <div class="item-price subtotal-price-text" id="prod_id_{{$id}}_subtotal_price_text">₹ {{ ($details['price']) }}</div>
                     </td>
                  </tr>
          ...
          $all_product_sub_total += ($details['price']);
      ...
      <div>
         ...
         <table>
            <thead>
               ...
               @if(!empty($details['price']))
                ...
                <th class="subtotal-price-text" id="all_product_sub_price_text">₹ {{ number_format($subtotal_price,2)}}</th>
                @endif
        ...
        @if(!empty($details['price']))
         ...
         <br><br>
         <a href="{{route('my_product_order')}}"><button type="button" class="btn btn-outline-primary rounded-full">Return Cart List</button></a>
