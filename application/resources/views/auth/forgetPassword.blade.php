...
<html lang="en" dir="ltr">
    <head>
        ...
        <title>BSN Mart Forgot Password Reset Link</title>
    ...
    >
		...
		<center id="alertMessageHide">@if ($message = Session::get('success'))
		   <font style="color: #f5f5f5;background-color:green  ;padding: 9px 52px;border-radius: 10px;">{{ $message }}</font>
		   @endif
		</center><br><br>
		<!-- Flash  Message  End  -->
                   <!-- <h2 class="mb-3 text-2xl font-bold">Forgot Password</h2>-->
                    <p class="mb-7">Enter your email to recover your ID</p>
                        ...
                        @endforeach 
                         <form class="space-y-5" method="post" action="{{route('forget.password.post')}}">
                         @csrf
                       
                          
                         <div>
                            <label for="passemail_addressword">E-Mail</label>
                            <input id="email_address" type="text" name="email" class="form-input" placeholder="Enter Your Email" />
                        </div>
