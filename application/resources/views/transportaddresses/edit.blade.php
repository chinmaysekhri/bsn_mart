...
@section('title','Edit Transport Address')
@section('content')
<div x-data="form">
   <ul class="flex space-x-2 rtl:space-x-reverse">
      <li>
         <a href="{{route('transportaddresses.index')}}" class="text-primary hover:underline"> Transport Address</a>
      </li>
...
<div class="grid grid-cols-1 gap-6 pt-5 lg:grid-cols-1">
   <div class="panel">
      <div class="mb-5">
         ...
         @endif
         {!! Form::model($transport_address, ['method' => 'PATCH','route' => ['transportaddresses.update', $transport_address->id],'class'=>'space-y-5', 'enctype'=>'multipart/form-data']) !!}
        ...
        <div>
         ...
         <select class="form-input form-select" name="buyer_id" id="buyer_id" required="">
                  <option value="">--Select Buyer Name--</option>
                   @foreach($buyerData as $trans_address)
                  <option value="{{ $trans_address->id }}" @if($transport_address->buyer_id == $trans_address->id) selected @endif>{{ $trans_address->first_name.' '.$trans_address->last_name }}({{$trans_address->email}})</option>
                  @endforeach
       ...
       <div>
        ...
        {!! Form::file('lr_copy_upload', array('placeholder' => 'Upload Aadhar Card Certificate','class' => 'form-input','id'=>'lr_copy_upload')) !!}
		    <a href="{{asset('public/uploads/transport_address/lr_copy_upload/'.$transport_address->lr_copy_upload)}}" target="_blank" >
									
                <img class="h-9 w-9 rounded-full object-cover saturate-50 group-hover:saturate-100" src="{{asset('public/uploads/transport_address/lr_copy_upload/'.$transport_address->lr_copy_upload)}}">
