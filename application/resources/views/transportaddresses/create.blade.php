...
@section('title','Add Transport Address')
@section('content')
<div x-data="form">
...
<div class="grid grid-cols-1 gap-6 pt-5 lg:grid-cols-1">
   <div class="panel">
      <div class="mb-5">
         ...
         @endif
         {!! Form::open(array('route' => 'transportaddresses.store','method'=>'POST','class'=>'space-y-5', 'enctype'=>'multipart/form-data')) !!}
          <div class="grid grid-cols-1 gap-6 pt-5 lg:grid-cols-2">
      <div>
         <label for="buyer_id">Select Buyer Name</label>
                 <select class="form-input form-select" name="buyer_id" id="buyer_id" required="">
                  <option value="">--Select Buyer Name--</option>
                  @foreach($buyerData as $transport_address)
                  <option value="{{ $transport_address->id }}">{{($transport_address->first_name.' '.$transport_address->last_name)}} ({{$transport_address->email}})</option>
                  @endforeach
