...
<div x-data="form">
   <ul class="flex space-x-2 rtl:space-x-reverse">
      <li>
         <a href="{{route('purchasereturns.index')}}" class="text-primary hover:underline">Purchase Return</a>
      </li>
            ...
            <div class="panel">
               <!-- browser default -->
                ...
                <!--  <form class="space-y-5" @submit.prevent="submitForm5()">-->
				  ...
				  <form class="space-y-5" action="{{route('purchasereturns.update',$purchasereturn->id)}}" method="Post">
	    	...
	    	<!--11-03-2025 for hold value after search data End -->
                     <div class="grid grid-cols-1 md:grid-cols-4 gap-5 mb-5">
                        <div>
                           ...
                           <input id="gridEmail" type="date" name="purchase_date" value="{{$purchasereturn->purchase_date}}" class="form-input" required />
                        ...
                        <div class="">
                           ...
                           <select class="form-select text-white-dark" id="seller_id" name="seller_id[]" onchange="sellerProductData(this);" required="required" />
                              ...
                              @foreach($sellerData as $seller)
							   ...
							   <option value="{{ $seller->id }}" @if(in_array($seller->id,json_decode($purchasereturn->seller_id, true))) selected @endif>{{($seller->first_name.' '.$seller->last_name)}}</option>
                        ...
                        <div class="">
                           ...
                           <input id="warehouseName" type="text" name="warehouse_name" value="{{$purchasereturn->warehouse_name}}" placeholder="Enter Warehouse Name" class="form-input" required="required"/>
                        </div>
               ...
               <div class="dataTable-wrapper dataTable-loading no-footer sortable searchable fixed-columns p-5">
				 ...
				 $purchasereturn_product_id = json_decode($purchasereturn->product_id, true);
				 $purchasereturn_product_quantity = json_decode($purchasereturn->product_quantity, true);
				 $purchasereturn_product_price = json_decode($purchasereturn->product_price, true);
				 $purchasereturn_total_product_price = json_decode($purchasereturn->total_product_price, true);
				 
				 @endphp
				 <input id="current_position" type="hidden" name="" value="{{count($purchasereturn_product_id)}}">
				 <input id="purchase_subtotal" type="hidden" name="purchase_subtotal" value="{{$purchasereturn->purchase_subtotal}}">
				 <input id="purchase_service_fee" type="hidden" name="purchase_service_fee" value="{{$purchasereturn->purchase_service_fee}}">
				 <input id="purchase_final_total" type="hidden" name="purchase_final_total" value="{{$purchasereturn->purchase_final_total}}">
				 ...
				 <table class="table" id="table">
					 ...
					 @foreach($purchasereturn_product_id as $purchasereturn_product_id_row)
                       
					   @php
					     //dd($purchasereturn_product_id);
					   	  $selected_purchaseProductData = Product::orderby('id', 'desc')
	                         ->where('id','=',$purchasereturn_product_id_row)  
							//->pluck('seller_id')			 
							->get();
					   ...
					   @endphp
                        <tr>
                           ...
                           <td>
                             <select class="form-select text-white-dark productID" id="productID_position_{{$pi}}" onchange="getProductData('get_product_name','{{$pi}}',this);" name="product_id[]" style="width: 124px;">
                                <option value="">Product Id</option>
								@foreach($purchaseProductData as $pname)
								<option value="{{$pname->id}}" @if($purchasereturn_product_id_row == $pname->id) selected @endif >PID00{{$pname->id}}</option>
								 @endforeach 
                              </select>
                           </td>
                           ...
                           <td>
                           <select class="form-select text-white-dark product_name" id="product_name_position_{{$pi}}" name="product_name[]" value="{{$purchasereturn->product_name}}" style="width: 140px;">
                          
						       <!-- <option value="">Product Name</option>-->
							   	
								<option value="" class="product_nane">Product Name</option>
								@foreach($selected_purchaseProductData as $pname)
								<option value="{{$pname->id}}" @if($purchasereturn_product_id_row == $pname->id) selected @endif >{{$pname->product_name}}</option>
								 @endforeach  
								 
                              </select>
                           </td>
                           <td>
                              <input type="text" name="product_quantity[]" placeholder="Quantity" onkeyup="calculateProdTotal('{{$pi}}',this);" value="{{$purchasereturn_product_quantity[$pi]}}" id="product_quantity_position_{{$pi}}" class="form-input product_quantity" />
                           </td>
                           <td>
                              <input type="text" name="product_price[]" value="{{$purchasereturn_product_price[$pi]}}"  placeholder="Rate" id="product_price_position_{{$pi}}" class="form-input product_price"/>
                           </td>
                           <td>
                              <!-- Total Amount Will Update Autoamatically -->
                              <input type="text"  class="form-input total_product_price" name="total_product_price[]" value="{{$purchasereturn_total_product_price[$pi]}}" id="total_product_price_position_{{$pi}}"  placeholder="Total">
                           ...
                           <td>
                              <div class="flex items-center font-semibold">
								 ...
								 @php
								   ...
								   if(!empty($pname)){
									  $product_photo =  'public/uploads/product/product_photo/'.$pname->product_photo;
								   }else{
									   $product_photo = "";
								   }
								 ...
								 @endphp
                                    ...
                                    <img class="h-8 w-8 rounded-full object-cover" id="product_img_position_{{$pi}}" src="{{asset($product_photo)}}" />
                                 </div>
                              </div>
                           </td>
                        </tr>
                  ...
                  <div class="flex justify-end px-4">
                     <div class="w-full md:w-80 font-semibold p-5">
                        <div class="flex items-center justify-between py-2">
                           <span>Sub Total :</span>
                           <span id="sub_total"> ₹{{$purchasereturn->purchase_subtotal}}</span>
                        </div>
                      <!--  <div class="flex items-center justify-between py-2">
                           <span>Service Fees :</span>
                           <span id="ser_fee">- ₹{{$purchasereturn->purchase_service_fee}}</span>
                        </div>-->
                        <div class="flex items-center justify-between font-bold py-4 border-t dark:border-t-dark border-dashed">
                           <span>Total :</span>
                           <span id="total_pro_price"> ₹{{$purchasereturn->purchase_final_total}}</span>
                        </div>
                        <!-- <table>                                <tr>
...
<script>
  ...
  function sellerProductData(element){
     ...
     var url = "{{ route('get_purchase_return_product_data') }}";
...
<script>
 ...
 function calculateProdTotal(position,element){
	...
	//var ser_fee = (sub_total*1.5/100);
	//var total_pro_price = (sub_total - ser_fee);
