...
<body>
                      ...
                      <p><b>Dear</b>{{ ucfirst($mailData['first_name'])}} {{ucfirst($mailData['last_name'])}} ,</p>
                       ...
                       <p>Welcome to BSN Mart! We are thrilled to have you as a valuable member of our team. As part of our commitment to enhancing your work experience, we are excited to inform you that you have been selected as <b>{{ ucfirst($mailData['designation'])}}</b> with the Annual salary of <b> ₹ {{ ($mailData['salary']*12)}}.00</b> and you will be reporting to <b>{{ ($mailData['managed_by_name']) }}</b> and <b>{{ ($mailData['managed_by_mobile']) }}</b>. We're excited to have you on board and look forward to working together to achieve great things. To get started, we have set up your Employee Portal account. This portal will serve as your centralized hub for managing your credentials and securely uploading all the necessary documents for your employment with us.</p>
                       <p><b>Your Employee Portal Credentials:</b></p>
                           <p><b>Username :</b> {{ $mailData['user_id'] }}</p>
                           <p><b>Password :</b> {{ $mailData['password'] }}</p>
                           
                        <p><b>Also find below your official email id and contact details:</b></p> 
                        <p><b>Login Url : </b> https://bsnmart.com/webmail/</p>
                        
                        @if(!empty($mailData['official_email']))
                        <p><b>Email Id : </b> {{ $mailData['official_email'] }}<p>
                        @else
                        <p><b>Email Id : </b>Not Applicable<p>
                        @endif   
                                
                        @if(!empty($mailData['official_password']))
                        <p><b>Password : </b> {{ $mailData['official_password'] }}<p>
                        @else
                        <p><b>Password : </b>Not Applicable<p>
                        @endif    
                         
                        @if(!empty($mailData['official_contact']))
                        <p><b>Contact No : </b> {{ $mailData['official_contact'] }} <p>
                        @else
                        <p><b>Contact No : </b>Not Applicable<p>
                        @endif  
                          
                       <p>Please keep this information secure, as it will be required to access your portal.</p>
                       <p><b> Access Your Employee Portal:</b></p>
                       <p><b>To access your Employee Portal, please follow these steps:</b></p>
                        <p><b>Step 1:</b> Visit the BSN Mart Employee Portal at https://bsnmart.com/application/login/ </p>
                        <p><b>Step 2:</b> Log in using the credentials provided above. You will be prompted to change your temporary password upon your first login for security reasons.</p>
                        <p><b>Step 3:</b> Click on Edit Employee Profile in the My Detail section under the My Profile tab & update Your Credentials and Documents as below:-</p>
