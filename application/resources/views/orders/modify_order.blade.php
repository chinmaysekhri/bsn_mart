...
<div x-data="form">
    <ul class="flex space-x-2 rtl:space-x-reverse">
        <li>
            <a href="{{route('orders.index')}}" class="text-primary hover:underline">Return Order List</a>
        </li>
...
<div x-data="form">
   <div class="panel">
      <div class="dataTable-wrapper dataTable-loading no-footer sortable searchable fixed-columns">
        ...
        <form action="{{route('update_modify_order',$modifyOrder->id)}}" method="post" id="checkout_form">
          @csrf
		  ...
		  @php
		   ...
		   $productData = Helper::getProductById($id);
		   
		   $details['product_name']= $productData->product_name;
		   $details['price']= $details['unit_price'];
           ...
           <td>
              <input type="hidden" value="{{ $details['price'] }}" id="prod_id_{{$id}}_price_val">
               ...
               <input type="hidden" class="single_product_subtotal" id="prod_id_{{$id}}_subtotal_price_val" value="{{ ($details['price']*$details['master_packing']) }}">
                        ...
                        <div class="item-price subtotal-price-text" id="prod_id_{{$id}}_subtotal_price_text">₹ {{ ($details['price']*$details['master_packing'])*$details['prod_qty'] }}</div>
                     </td>
                  </tr>
          ...
          $all_product_sub_total += ($details['price']*$details['master_packing']*$details['prod_qty']);
      ...
      @endif
        ...
        @if(!empty($details['price']))
         ...
         <br><br>
         <a href="{{route('orders.index')}}"><button type="button" class="btn btn-outline-primary rounded-full">Return Order List</button></a>
     
         <a onclick="submitCheckout();" href="#"><button type="button" class="btn btn-outline-primary rounded-full" style="margin-left:850px;margin-top: -43px;">Modify Order</button></a>
      </div>
   </div>
</div>
