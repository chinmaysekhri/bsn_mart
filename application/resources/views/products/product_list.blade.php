      ...
      @csrf
         <div class="mb-5" >
            <div class="mb-5 grid grid-cols-1 gap-4 sm:grid-cols-3">
               ...
               <div style="margin-top: 31px;">
                  <div class="search-date-group ms-5 d-flex align-items-center">
                     <select  class="form-select text-white-dark"  id="subcategory_name"  name="subcat_id" style="width:320px;" onchange="getBrandData(this);">
		 ...
		 @if(count($productList) >0)
          ...
          @foreach($productList as $product) 
		   @php
			...
			$productReviewAvg = Review::where('product_id','=',$product->id)->where('status','=','Accept')->avg('review_rating');
...
<?php   /*  @php
        ...
        <!-- Avrage delivery Time code ATD Code 21-01-2025  Start--> 
         @php
		   
		 $ind = 0;

         $productOrders     = ProductOrder::where('product_id','=',$product->id)->get()->toArray(); 
		 
	     $deliveredProduct = DeliveredProduct::where('product_id','=',$product->id)->get()->toArray(); 
		 ...
		 $fromDate = Carbon::parse($order['created_at']);
		   ...
		   if(!empty($deliveredProduct[$ind]['created_at'])){
			   
			  $toDate = Carbon::parse($deliveredProduct[$ind]['created_at']); 
	          ...
	          $totalDays += ($toDate->diffInMinutes($fromDate)/(60*24)+1);
			  //dd($totalDays);
			  unset($fromDate);
			  unset($toDate);
	      }
          
		  $productCount = count($productOrders);
		   
		   if($productCount > 0){
		   
		   $avgd  = round($totalDays/$productCount);
		...
		//dd($productOrders,$deliveredProduct,$totalDays,$avgd,$productCount);
		
		 $getSellerBusinessName = Helper::getSellerName($product->seller_id);
		 
		   @endphp
		   
            <div class="" style="text-align:right; margin:top"><b>ADT : </b> {{ $avgd }} Days</div>
            
            <br>

            <div class="" style="text-align:right; margin:top">
                  <b>RP :</b> 0 %
            </div>

			<!--Avrage delivery Time code ATD Code 21-01-2025 End -->   
			...
			<div class="social-btn-sp" style="text-align:right; margin:top">
               ...
               <div class="">Product ID : <b>PID00{{ $product->id }}</b></div>
               ...
               <div class="">Model Number : <b>{{ $product->model_number}}</b></div>
				 ...
				 @if(!empty($getSellerBusinessName->business_name))
				  
                  <div class="">Business Name : <b>{{ ucwords($getSellerBusinessName->business_name)}}</b></div>
			      
			     @endif
				  
			    @endif
			   
			   @if(empty($product->other_brand))
				   
               <div class="">Brand Name   : <b>{{ ucwords($product->brand_name) }}</b></div>
			   
			   @elseif($product->other_brand =="Brand")
            ...
            @if(!empty($product->warranty_period))
                ...
                <div class="">Warranty  : <b>{{ $product->warranty_period }} Month /{{ $product->product_guarantee_type }}/{{ $product->only_spare_parts }}/{{ $product->product_warranty_type }}</b></div>
            ...
            <div class="mb-1 font">Launch Date  : <b>{{ date('d M y', strtotime($product->launch_date)) }}</b></div>
             
               <div class="my-4">
                   
                <div class="mb-1 font-bold text-success">₹ <b>{{$product->price}} /- {{$product->packaging_charges}}</b></div>
					
                <div class="">Master Packing : <b>{{$product->master_packing}}</b></div>
               
                @if(!empty($product->carton_packing))
                
				<div class="">Carton Packing : <b>{{$product->carton_packing}}</b></div>
               ...
               <div class="mt-8">
                 ...
                 @for($i=0; $i < $productReviewStar; $i++)
                  ...
                  <div class="ltr:ml-2 rtl:mr-2">( <strong>{{number_format($productReviewAvg,1)}}</strong> Customer Review )</div>
			...
			<h1 style="color:red; text-align:center">No Record Found!!</h1>
			 ...
			 <!--  Suggestion Modelpupop 02-01-2025 Start -->
                        <div x-data="modal" class="">
                           ...
                           <button class="badge bg-info inline-flex" href="#" type="button" @click="toggle" style="margin-left:444px;">Suggest Product</button>
                              ...
                              <div class="flex items-start justify-center min-h-screen px-4">
                                 <div x-show="open" x-transition x-transition.duration.300 class="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8">
                                    <div class="flex items-center justify-between bg-[#fbfbfb] px-5 py-3 dark:bg-[#121c2c]">
                                       <h5 class="text-lg font-bold">Suggest Product</h5>
                                    ...
                                    <form action="{{route('suggest_product')}}" method="POST" enctype="multipart/form-data">
                                       ...
                                       <div class="p-5">
										  ...
										  <div>
											 ...
											 {!! Form::text('suggest_product_name', null, array('placeholder' => 'Enter Product Name','class' => 'form-input','id'=>'product_name')) !!}
										  </div>
										  <div>
											 <label for="price_range">Price Range:</label>
											 {!! Form::text('suggest_price_range', null, array('placeholder' => 'Enter price range','class' => 'form-input','id'=>'price_range')) !!}
										  </div>
										  <div>
											 <label for="brand_name">Brand Name(if any):</label>
											 {!! Form::text('suggest_brand_name', null, array('placeholder' => 'Enter Brand Name','class' => 'form-input','id'=>'brand_name')) !!}
										  </div>
										  
										  <div>
											 <label for="monthly_requirement">Monthly Requirement:</label>
											 {!! Form::text('monthly_requirement', null, array('placeholder' => 'Enter Monthly Requirement','class' => 'form-input','id'=>'monthly_requirement')) !!}
										  </div>
										  
										  <div>
											 <label for="upload_withdrawal_receipt">Product Image Upload:</label>
											 {!! Form::file('suggest_product_img', null, array('placeholder' => 'Upload Withdrawal Receipt','class' => 'form-input','id'=>'upload_withdrawal_receipt')) !!}
										  </div>
...
function getProductSearchData(element){
        ...
        var product_tag = element.value;
        var product_description = element.value;
       //alert(product_name);
      ...
      var url = "{{ route('get_product_search_data') }}";
        ...
        url:url,
        type: 'POST',
        data: { _token :token,product_name:product_name,product_tag:product_tag,product_description:product_description},
        success:function(result){
