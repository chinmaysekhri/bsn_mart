            ...
            <div class="panel">
               <!-- browser default -->
                ...
                <!--  <form class="space-y-5" @submit.prevent="submitForm5()">-->
				  ...
				  <form class="space-y-5" action="{{route('purchases.update',$purchase->id)}}" method="Post">
				  ...
				  @csrf
                     <div class="grid grid-cols-1 md:grid-cols-4 gap-5 mb-5">
                        <div>
                           ...
                           <input id="gridEmail" type="date" name="purchase_date" value="{{$purchase->purchase_date}}" class="form-input" required />
                        ...
                        <div class="">
                           ...
                           <select class="form-select text-white-dark" id="seller_id" name="seller_id[]" onchange="sellerProductData(this);" required="required" />
                              ...
                              @foreach($sellerData as $seller)
							   ...
							   <option value="{{ $seller->id }}" @if(in_array($seller->id,json_decode($purchase->seller_id, true))) selected @endif>{{($seller->first_name.' '.$seller->last_name)}}</option>
                        ...
                        <div class="">
                           ...
                           <input id="warehouseName" type="text" name="warehouse_name" value="{{$purchase->warehouse_name}}" placeholder="Enter Warehouse Name" class="form-input" required="required"/>
                        </div>
               ...
               <div class="dataTable-wrapper dataTable-loading no-footer sortable searchable fixed-columns p-5">
				 ...
				 $purchase_product_id = json_decode($purchase->product_id, true);
				 $purchase_product_quantity = json_decode($purchase->product_quantity, true);
				 $purchase_product_price = json_decode($purchase->product_price, true);
				 $purchase_total_product_price = json_decode($purchase->total_product_price, true);
				 
				 @endphp
				 <input id="current_position" type="hidden" name="" value="{{count($purchase_product_id)}}">
				 <input id="purchase_subtotal" type="hidden" name="purchase_subtotal" value="{{$purchase->purchase_subtotal}}">
				 <input id="purchase_service_fee" type="hidden" name="purchase_service_fee" value="{{$purchase->purchase_service_fee}}">
				 <input id="purchase_final_total" type="hidden" name="purchase_final_total" value="{{$purchase->purchase_final_total}}">
				 ...
				 <table class="table" id="table">
					 ...
					 @foreach($purchase_product_id as $purchase_product_id_row)
                       
					   @php
					   //dd($purchase_product_id);
					   	  $selected_purchaseProductData = Product::orderby('id', 'desc')
	                         ->where('id','=',$purchase_product_id_row)  
							//->pluck('seller_id')			 
							->get();
                        ...
                        <tr>
                           ...
                           <td>
                             <select class="form-select text-white-dark productID" id="productID_position_{{$pi}}" onchange="getProductData('get_product_name','{{$pi}}',this);" name="product_id[]" style="width: 124px;">
								...
								<option value="{{$pname->id}}" @if($purchase_product_id_row == $pname->id) selected @endif >PID00{{$pname->id}}</option>
								 @endforeach 
                              </select>
                           </td>
                           ...
                           <td>
                           <select class="form-select text-white-dark product_name" id="product_name_position_{{$pi}}" name="product_name[]" value="{{$purchase->product_name}}" style="width: 140px;">
						       ...
						       <option value="">Product Name</option>
								...
								<option value="{{$pname->id}}" @if($purchase_product_id_row == $pname->id) selected @endif >{{$pname->product_name}}</option>
								 @endforeach  
                           ...
                           <td>
                              <input type="text" name="product_quantity[]" placeholder="Quantity" onkeyup="calculateProdTotal('{{$pi}}',this);" value="{{$purchase_product_quantity[$pi]}}" id="product_quantity_position_{{$pi}}" class="form-input product_quantity" />
                           </td>
                           <td>
                              <input type="text" name="product_price[]" value="{{$purchase_product_price[$pi]}}"  placeholder="Rate" id="product_price_position_{{$pi}}" class="form-input product_price"/>
                           </td>
                           <td>
                              <!-- Total Amount Will Update Autoamatically -->
                              <input type="text"  class="form-input total_product_price" name="total_product_price[]" value="{{$purchase_total_product_price[$pi]}}" id="total_product_price_position_{{$pi}}"  placeholder="Total">
                  ...
                  <div class="flex justify-end px-4">
                     <div class="w-full md:w-80 font-semibold p-5">
                        <div class="flex items-center justify-between py-2">
                           <span>Sub Total :</span>
                           <span id="sub_total"> ₹{{$purchase->purchase_subtotal}}</span>
                        </div>
                        <div class="flex items-center justify-between py-2">
                           <span>Service Fees :</span>
                           <span id="ser_fee">- ₹{{$purchase->purchase_service_fee}}</span>
                        </div>
                        <div class="flex items-center justify-between font-bold py-4 border-t dark:border-t-dark border-dashed">
                           <span>Total :</span>
                           <span id="total_pro_price"> ₹{{$purchase->purchase_final_total}}</span>
                        </div>
                        <!-- <table>                                <tr>
...
+'</option>');
		  ...
		  $('#product_price_position_'+result.position).val(value.price);
		  $('#product_quantity_position_'+result.position).val('1');
		  $('#total_product_price_position_'+result.position).val(value.price);
      //    $('.productID').append('<option value="'+value.id+'">PID00'+value.id+'</option>');
...
<script>
 ...
 function calculateProdTotal(position,element){
	...
	var ser_fee = Math.round(sub_total*1.5/100);
	var total_pro_price = Math.round(sub_total - ser_fee);
