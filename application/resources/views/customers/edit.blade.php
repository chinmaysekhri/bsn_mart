...
@section('title','Edit Customer Lead')
@section('content')
<div x-data="form">
   ...
   <div class="grid grid-cols-1 gap-6 pt-5 lg:grid-cols-1">
	 ...
	 <!-- Flash Error Message  start -->
      ...
      <!-- Flash Error Message  End  -->
         ...
         {!! Form::model($customer, ['method' => 'PATCH','route' => ['customers.update', $customer->id],'class'=>'space-y-5', 'enctype'=>'multipart/form-data']) !!}
         @csrf
           <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
               ...
               {!! Form::select('state',[$customer->state=>$customer->state],[], array('class' => 'form-select text-white-dark','id'=>'state')) !!}
            </div>
            <div>
               <label for="City">City</label>
               {!! Form::select('city', [$customer->city=>$customer->city],[], array('class' => 'form-select text-white-dark','id'=>'city')) !!}
            </div>
             <br>
            <div>
               ...
               <select class="form-select text-white-dark" id="product_type" name="product_type"  onchange="myPhysicalFunction();">
                  ...
                  <option value="Virtual" {{ old('product_type') == 'Virtual' ? 'selected' : '' }}>Virtual</option>
			...
			<div>
               ...
               <select class="form-select text-white-dark" id="product_name" name="product_name" onchange="getProductDetail(this);" value="{{ old('product_name') }}">
            ...
            <div>
             ...
             <div  class="grid grid-cols-1 gap-4 sm:grid-cols-1" id="myVirtualDIV">
               ...
               {!! Form::text('slot', '', array('placeholder' => 'Slots','class' => 'form-input','id'=>'slot')) !!}
               </div>
             </div>
            ...
            <div>
               ...
               {!! Form::text('investment', '', array('placeholder' => 'investment','class' => 'form-input','id'=>'investment')) !!}
            </div>
			
			<div>
               <label for="guaranteed_profit">Guaranteed Profit</label>
               {!! Form::text('guaranteed_profit', '', array('placeholder' => 'Guaranteed Profit','class' => 'form-input','id'=>'guaranteed_profit')) !!}
            </div>
			...
			<div>
               ...
               <select class="form-select text-white-dark" id="gst_no" name="gst_no" onchange="getProductCostWithGst(this);" required/>
                  ...
                  <option value="Yes" {{ old('gst_no') == 'Yes' ? 'selected' : '' }}>Yes</option>
                  <option value="No" {{ old('gst_no') == 'No' ? 'selected' : '' }}>No</option>
               </select>
            </div>
			
            <div>
               <label for="total">Total</label>
               {!! Form::text('total', '', array('placeholder' => 'Total','class' => 'form-input','id'=>'total')) !!}
            </div>
            ...
            <div>
               ...
               {!! Form::text('final_cost_of_product', '', array('placeholder' => 'Final Cost','class' => 'form-input','id'=>'final_cost_of_product')) !!}
            </div>
             ...
             <option value="">Select Final Status</option>
                  <option value="Fresh Lead" {{ old('status') == 'Fresh Lead' ? 'selected' : '' }}>Fresh Lead</option>
                  <option value="Pending" {{ old('status') == 'Pending' ? 'selected' : '' }}>Pending</option>
                  <option value="Intrested" {{ old('status') == 'Intrested' ? 'selected' : '' }}>Intrested</option>
                  <option value="Not Intrested" {{ old('status') == 'Not Intrested' ? 'selected' : '' }}>Not Intrested</option>
                  <option value="Virtual Meet" {{ old('status') == 'Virtual Meet' ? 'selected' : '' }}>Virtual Meet</option>
                  <option value="Line Up" {{ old('status') == 'Line Up' ? 'selected' : '' }}>Line Up</option>
                  <option value="Turn Up" {{ old('status') == 'Turn Up' ? 'selected' : '' }}>Turn Up</option>
                  <option value="Fake Lead" {{ old('status') == 'Fake Lead' ? 'selected' : '' }}>Fake Lead</option>
                  <option value="Loan Applied" {{ old('status') == 'Loan Applied' ? 'selected' : '' }}>Loan Applied</option>
                  <option value="Loan Approved" {{ old('status') == 'Loan Approved' ? 'selected' : '' }}>Loan Approved</option>
                  <option value="Loan Rejected" {{ old('status') == 'Loan Rejected' ? 'selected' : '' }}>Loan Rejected</option>
                  <option value="In Follow Up" {{ old('status') == 'In Follow Up' ? 'selected' : '' }}>In Follow Up</option>
                  <option value="Payment Link" {{ old('status') == 'Payment Link' ? 'selected' : '' }}>Payment Link</option>
                  <option value="Order Reserved" {{ old('status') == 'Order Reserved' ? 'selected' : '' }}>Order Reserved</option>
                  <option value="Order Made" {{ old('status') == 'Order Made' ? 'selected' : '' }}>Order Made</option>
               </select>
            </div>
          ...
          <div id="payment_info_hide" id="myDIV" style="display: none;"> 
            ...
            <label for="follow_up_date" >Follow Up Date</label>
              ...
              {!! Form::date('follow_up_date', date('Y-m-d'), array('placeholder' => 'follow Up Date','class' => 'form-input','id'=>'inputdate')) !!}
			  
            </div>
			
            <div>
               <label for="Comment">Feedback</label>
               <textarea class="form-input" id="feedback" name="feedback" rows="2" cols="50" placeholder="My first Payment ......" required="">{{{ Request::old('feedback') }}}</textarea>
            </div>
			
			<div>
               <label for="due_amount"> Due Amount</label>
              <input type="text" class="form-input"  id="due_amount" readonly>
	  ...
	  var customerId = '{{ $customer->id }}';
       ...
       var url = "{{ route('get_product_detail') }}";
   	 ...
   	 $.ajax({
   		url:url,
   		type: 'POST',
   		data: { _token :token,productId:productId,customerId:customerId },
   		success:function(result){
			
			var productDetail =  result.product_detail;
			var discountApplication =  parseFloat(productDetail.discount_application);
			//alert(discountApplication);
			var discountHtml =  '<option value="">Select Discount</option>';
			
			for (let i = 0; i <= discountApplication; i++) {
			...
			var customerFirstPayment = result.customerFirstPayment;
			var paid_amount = result.amount_paid;
			
			$('#totalPaidAmount').val(paid_amount);
			...
			if(customerFirstPayment.final_cost_of_product !=0){
				...
				var final_cost_of_product = customerFirstPayment.final_cost_of_product;
				var total = customerFirstPayment.total;
				var due_amount = final_cost_of_product - paid_amount;
				
				var gst_no = customerFirstPayment.gst_no;
				var discount = customerFirstPayment.discount;
				
				$('#gst_no').val(gst_no).change();
				$('#discount').val(discount).change();
				//alert(discount);
				$('#total').val(total);
				$('#final_cost_of_product').val(final_cost_of_product);
...
<script>
 ...
 function getDiscountProductFinalCost(element){
	   ...
	   var paid_amount = parseFloat($('#totalPaidAmount').val());
	   
	   var total_discount_price = Math.round(total_price - (total_price*productDiscount/100));
	  
	   var due_amount = total_discount_price - paid_amount;
