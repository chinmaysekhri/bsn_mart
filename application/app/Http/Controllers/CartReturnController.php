...
//comment date 18-12-2024
 ...
 //Return cart 27-11-2024
     ...
     public function updateReturnStatus(Request $request, $id)
	{
	   ...
	   return back()->with('success','Return Cart Update Status Successfully!');
   }
  ...
  public function receivedProduct(Request $request ,$cart_return_id, $product_id){
	...
	$crretData = ReceivedProduct::where('cart_return_id','=',$input['cart_return_id'])
	                 ->where('product_id','=',$input['product_id'])
					 ->first();
					 
	if(!empty($crretData)){
		
		ReceivedProduct::where('cart_return_id','=',$input['cart_return_id'])
	                 ->where('product_id','=',$input['product_id'])
					 ->update($input);
 ...
 //07-02-2025
  ...
  public function updateReceivedQuantityStatus(Request $request ,$cart_return_id, $product_id)
	{
	   $input      = $request->all(); 
	   //dd($input);
	   $auth_user  = Auth::user();
	   
	   $updateReceivedData = ReceivedProduct::where('cart_return_id','=',$cart_return_id)
	                 ->where('product_id','=',$product_id)
					 ->first();
		//dd($updateReceivedData,$input);			 
		if($request->return_status=='Accept'){
			
			$total_accept_price = ($updateReceivedData->received_product_qty*$request->accept_price);
			
		    ReceivedProduct::where('product_id','=',$product_id)->where('cart_return_id','=',$cart_return_id)->update(['return_status'=>$request->return_status,'accept_price'=>$request->accept_price,'total_accept_price'=>$total_accept_price,'comment'=>$request->comment]);
		
			
		}elseif($request->return_status=='Repaired'){
			
			$total_repaired_price = ($updateReceivedData->received_product_qty*$request->repaired_price);
	        
			ReceivedProduct::where('product_id','=',$product_id)->where('cart_return_id','=',$cart_return_id)->update(['return_status'=>$request->return_status,'repaired_price'=>$request->repaired_price,'total_repaired_price'=>$total_repaired_price,'comment'=>$request->comment]);
		
		
		
		}elseif($request->return_status=='Reject'){
		
		ReceivedProduct::where('product_id','=',$product_id)->where('cart_return_id','=',$cart_return_id)->update(['return_status'=>$request->return_status,'comment'=>$request->comment]);
	  ...
	  // ReceivedProduct::where('product_id','=',$product_id)->where('cart_return_id','=',$cart_return_id)->update(['return_status'=>$request->return_status,'accept_price'=>$request->accept_price,'repaired_price'=>$request->repaired_price]);
		
	   return back()->with('success','Received Quantity Status Update Successfully!');
   }
