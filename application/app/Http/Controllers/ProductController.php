 ...
 /*   function __construct()
    ...
    /*    if($is_admin == 'super_admin'){
        ...
        $buyerSellerData   = User::where('for','=','seller')
          ...
          // code by seller itself data list in serch dropdown 
       ...
       ->when($request->today_applied_status,function (Builder $builder) use ($request) {
	    ...
	    // ->whereNotIn('user_product_status',['On Hold'])
         ...
         if($is_admin == 'super_admin'){
			...
			//$user_ids = User::where('created_by','=',$auth_user->id)->orWhere('managed_by','=',$auth_user->id)->pluck('seller_id')->toArray();
			  $user_ids = Seller::where('created_by','=',$auth_user->id)->orWhere('managed_by','=',$auth_user->id)->pluck('id')->toArray();
			 ...
			 ->orWhere(function ($filtered_table) use ($request,$user_ids){ 
               $filtered_table->whereIn('seller_id',$user_ids)
			  ->whereNotIn('product_status',['On Hold','Out Of Stock'])
	          ->whereNotIn('user_product_status',['On Hold'])
	          
	         // Code add on 12-03-2025
				->where(function ($filtered_table) use ($request,$user_ids){ 
					
					$filtered_table->when($request->q,function (Builder $builder) use ($request) {
	        ...
	        // for data search code Start 29-01-2025
         ...
         //for data search code 29-01-2025 end
		...
		->paginate($per_page)->appends($request->query());
         ...
         //$data = $data_collection->whereIn('created_by',$user_ids)->orWhere('seller_id','=',$auth_user->seller_id)->paginate($per_page);
           $data = $data_collection->where('seller_id',$auth_user->seller_id)
           
           			// code add on 12-03-2025
           			
					        	//->where(function ($filtered_table) use ($request,$user_ids){ 
						        ->where(function ($filtered_table) use ($request){ 
        ...
        return view('products.index',compact('requested_input','data','buyerSellerData','reqData'))
    ...
    public function create()
      {
		$auth_user    = Auth::user();	
			...
			$sellerData         = Seller::where('id','=',$sellerData->id)
			                              ->where('status','!=',0)
				                          ->whereNull('deleted_at')
			                              ->orderBy('id','DESC')->get();
			...
			$categoryData       = Categories::whereIn('id',$sellerCategoryIDS)->orderBy('id','DESC')->get();
				...
				$sellerData    = Seller::where('status','!=',0)
				                         ->whereNull('deleted_at')
				                         ->orderBy('id','DESC')->get();
    ...
    public function edit(string $id)
      {
	   ...
	   // 29-10-2024 for hold value after search next action date
        ...
        return view('products.edit',compact('product','sellerData','categoryData','subCategoryData','requested_input'));
    ...
    public function update(Request $request, string $id)
      ...
      /*  $this->validate($request, [
        ...
        if($request->hasfile('product_photo')){
         ...
         return redirect()->route('products.index',$requested_input)->with('success','Product updated successfully');
      }
   ...
   public function getSellerData(Request $request)
    ...
    public function productList(Request $request)
    {
     ...
     $per_page = 10;
		 ...
		 $productListCollection = Product::select('products.*')
		                         ->distinct()
		                         ->leftJoin('exclusive_products','exclusive_products.product_id','=','products.id')
					             ->when($request->q,function (Builder $builder) use ($request,$auth_user) {
									 
                                  $builder->where('product_name', 'like', "%{$request->q}%")
									  ->where('product_tag', 'like', "%{$request->q}%")
									  ->where('product_description', 'like', "%{$request->q}%")
								 	  ->where('exclusive_products.buyer_seller_id','=',$auth_user->id)
								...
								//17-01-2025
								  ->orWhere(function($query) use ($request,$auth_user) {
									  ...
									  $product_search_data = Product::select('products.*','exclusive_products.product_id')
									  ->leftJoin('exclusive_products','exclusive_products.product_id','=','products.id')
									  ->where('products.product_name', 'like', "%{$request->q}%")
									  ->where('products.product_tag', 'like', "%{$request->q}%")
									  ->where('products.product_description', 'like', "%{$request->q}%")
									  ->first();
									  
									  if(empty($product_search_data->product_id) || $auth_user->id == 1){
								...
								->orWhere(function($query) use ($request,$auth_user) {
									$query->where('product_name', 'like', "%{$request->q}%")
									  ...
									  ->where('product_description', 'like', "%{$request->q}%")
									   ->where('exclusive_products.buyer_seller_id','=',$auth_user->id)
									  //->where('products.buYer_seller_status','=','Active')
									;
					    ...
					    ->where(function($query) use ($request,$auth_user) {
									 
									  $not_exc_prod_ids2 = ExclusiveProduct::where('buyer_seller_id','!=',$auth_user->id)->where('district','!=',$auth_user->district)->pluck('product_id')
									                                        ->toArray();
									                                        
									  $exc_prod_ids = ExclusiveProduct::where('buyer_seller_id','=',$auth_user->id)->where('district','=',$auth_user->district)->pluck('product_id')
									                                        ->toArray();
									  $not_exc_prod_ids = array_diff($not_exc_prod_ids2,$exc_prod_ids);                                  
									  // dd($not_exc_prod_ids,$auth_user, $exc_prod_ids);
									  if($auth_user->id == 1){
										  
									  }else{
										 $query->whereNotIn('products.id', $not_exc_prod_ids); 
									  }
	 ...
	 //$productList2 = $productListCollection->get()->toArray();
   ...
   //23-11-2024 Start
	...
	//23-11-2024 end
	 ...
	 if(!empty($request->q)){
		...
		$subcategory_ids = $productList2->pluck('subcat_id')->toArray();
		//dd($subcategory_ids);
		   if(!empty($request->subcat_id)){
			 ...
			 $subCategoryData = SubCategories::where('category_id','=',$category_id)->whereIn('id',$subcategory_ids)->orderBy('id','ASC')->get();
		   ...
		   //dd($subcategory_ids);
		    if(!empty($request->subcat_id)){
			 ...
			 $productList = $productListCollection->whereNotIn('product_status',['On Hold','Out Of Stock'])
	                                              ->whereNotIn('user_product_status',['On Hold'])
	                                              ->whereJsonDoesntContain('black_listed_district',[$auth_user->district])
										          ->paginate($per_page)->appends($request->query());
	...
	if(isset($reqData['cat_id'])){
	 ...
	 if(!empty($request->q)){
	   ...
	   $productList = $productListCollection->whereNotIn('product_status',['On Hold','Out Of Stock'])
	                                        ->whereNotIn('user_product_status',['On Hold'])
	                                        ->whereJsonDoesntContain('black_listed_district',[$auth_user->district])
                                          //  ->whereNotIn('district',[$auth_user->district])
	                                        ->paginate($per_page)->appends($request->query());
...
/* 		 	if($is_admin == 'super_admin'){
    ...
    public function getBrandData(Request $request)
    {
			...
			//dd($input);
			  ...
			  $subCatID     = $input['subcat_id'];
			  
		      //$brand_data    = Product::where('subcat_id','=',$subCatID)->groupBy('brand_name')->pluck('brand_name')->toArray(); 
			 $brand_data    = Product::where('subcat_id','=',$subCatID)
			                            ->whereNotIn('product_status',['On Hold','Out Of Stock'])
	                                    ->whereNotIn('user_product_status',['On Hold'])
			                            ->groupBy('brand_name')->pluck('brand_name')->toArray();
...
/* 		 	if($is_admin == 'super_admin'){
			...
			// dd($brand_data);	     
		     ...
		     $brand_data    = Product::where('subcat_id','=',$subCatID)
			                                  ->where('created_by','=',$auth_user->id)->groupBy('brand_name')
											  ->pluck('brand_name')
											  ->toArray();
			} */
		//dd($brand_data);
 ...
 //Add To Cart Functionality method Start date 01-12-2023
   ...
   public function checkout(Request $request)
	...
	/* comment 07-03-2025	 if($auth_user->for == 'buyer'){
   ...
   public function buyNow(Request $request)
    {
	  ...
	  // for product stock maintain date 21-01-2025 Start
            ...
            $product_order_date  = date('d-m-Y');
			
			$ipq = 0;

			foreach($cart_buy_data['cart_product_detail_arr'] as $cart_order){

			if(!empty($cart_order)){

			$cart_order_data = $cart_order;
			...
			$input_create = ['product_id'=>$cart_order_data['product_id'],
							 'order_product_qty'=>$cart_order_data['prod_qty'], 
							 'single_prod_sub_total'=>$cart_order_data['single_prod_sub_total'],
							 'unit_price'=>$cart_order_data['unit_price'],
							 'master_packing'=>$cart_order_data['master_packing'],
							 'product_photo'=>$cart_order_data['product_photo'],
							 'product_order_date'=>$product_order_date,
							 'order_id'=>$buynow->id];
            
			ProductOrder::create($input_create);
	  ...
	  // for product stock maintain date 21-01-2025 End
   ...
   public function suggestProduct(Request $request)
    {
       ...
       if($request->hasfile('suggest_product_img')){
            
            $fileName_suggest_product_img = time().'.'.$request->suggest_product_img->extension();  

            $request->suggest_product_img->move(public_path('uploads/suggetproduct/suggest_product_img'), $fileName_suggest_product_img);
            
            $input['suggest_product_img'] = $fileName_suggest_product_img;
            
        }else{

            unset($input['suggest_product_img']);          
        }
       
        SuggestProduct::create($input);
	  
	   return back()->with('success','Suggest Product Request Created Successfullly !!');
}
   ...
   public function suggestProductList(Request $request)
	{
     ...
     $suggestProduct = SuggestProduct::orderBy('id','DESC')->paginate($per_page);
	 
     return view('products.suggest_product_list',compact('suggestProduct'));
	
    }


  public function updateExclusiveStatus(Request $request, $id)
	{
	   $input      = $request->all(); 
	  
	   $auth_user  = Auth::user();
	   
	   DB::table('suggest_products')->where('id','=',$id)->update(['status'=>$request->status,'comment'=>$request->comment]);
		
	   return back()->with('success','Suggest Product Status Update Successfully!');
   }  
   

  public function productExclusive(Request $request, $product_id)
	{
	  ...
	  $input                    = $request->all();
       ...
       $input['product_id']      = intval($product_id);	  
       
       $input['buyer_seller_id'] = intval($request->buyer_seller_id);
       
       $userID            = intval($request->buyer_seller_id);

       //get seller name and district 08-01-2025

       $userData          =  User::where('id','=',$userID)->first();

       $input['district'] =  $userData->district; 
   
       $input['seller_buyer_name'] =  $userData->first_name.' '.$userData->last_name;

       $selleBuyerName   =  ($userData->first_name.' '.$userData->last_name);
	   
	   $exclusiveProductDistrict = ExclusiveProduct::where('buyer_seller_id','=',$userID)->where('product_id','=',$product_id)->first();
	   
	   $exclusiveProductDistrict2 = ExclusiveProduct::where('district','=',$userData->district)->where('product_id','=',$product_id)->first();
	   
	   //dd($pid , $exclusiveProductDistrict->product_id);
	  
	  if(empty($userData->district)){
          
		  return back()->with('success','Please Update Seller Or Buyer District Name!!');
	  
	   }
	
		elseif(!empty($exclusiveProductDistrict2)){
	 
	 
		  return back()->with('success','This product has already been assigned to other user in your district !!');  
	  	  
        }
		
	   
	   if(empty($exclusiveProductDistrict)){
          
		    ExclusiveProduct::create($input);
		  
		    //Product::where('id','=',$pid)->update(['exclusive_district'=>$userData->district,'seller_buyer_name'=>$selleBuyerName]);
          
		    return back()->with('success','Execlusive Product Successfullly !!');
	  
	    }else{
			
			ExclusiveProduct::where('id','=',$exclusiveProductDistrict->id)->update(['district'=>$userData->district]);
			
			//Product::where('id','=',$pid)->update(['exclusive_district'=>$userData->district,'seller_buyer_name'=>$selleBuyerName]);
			
			return back()->with('success','Execlusive District Updated Successfullly !!');
		}
...
//14-02-2025
	public function updatePlateformFeeStatus(Request $request, $id)
	{
	   $input = $request->all(); 
	   ...
	   $product_seller_price = $product->seller_price;
	
	   $product_price        = $product->price;
		
	   if($product->plateform_fee_status=='Yes'){
		
		   $plateformRemove = $product->seller_price;
		
	       Product::where('id','=',$id)->update(['plateform_fee_status'=>'No','price'=>$plateformRemove]);
		
		}elseif($product->plateform_fee_status=='No'){
			
		  $plateformFee =($product_seller_price+$product_seller_price*2/100);
		
	      Product::where('id','=',$id)->update(['plateform_fee_status'=>'Yes','price'=>$plateformFee]);
		
		}
	   return back()->with('success','Plateform Fee Status Update Successfully!');
   }
//14-02-2025
